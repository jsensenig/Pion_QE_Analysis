...
void Pion_QE_MC_Selection::ReadData( std::unique_ptr<Reader> & rdr ) {
  ...
  while ( rdr->Next() ) {
    ...
    _hists.th1_hists["h_prim_ke_all"] -> Fill( rdr->primaryKineticEnergy.At(c_plane_) );
    ...
    if ( pi > -1 ) {//Selections::IsTruthPi2Pi( rdr ) ) {
      std::cout << "Pion idx " << pi << std::endl;
      std::cout << "Run " << *rdr->run << std::endl;
      std::cout << "Sub-Run " << *rdr->subrun << std::endl;
      std::cout << "Event " << *rdr->event << std::endl;
    ...
    _hists.th1_hists["h_prim_ke_pionqe"] -> Fill( rdr->primaryKineticEnergy.At(c_plane_) );
...
void Pion_QE_MC_Selection::ProcessPrimary( std::unique_ptr<Reader> & rdr ) {
  ...
  int chi2_pdg = rdr->primaryPID_Pdg.At(c_plane_);
  ...
  if ( *rdr->primary_truth_Pdg == utils::pdg::kPdgPiP ) { // primary pions
    _hists.th1_hists["h_prim_pida_pi"] -> Fill( rdr->primaryPID_PIDA.At(c_plane_) );
    ...
    _hists.th1_hists["h_prim_ke_pion"] -> Fill( rdr->primaryKineticEnergy.At(c_plane_) );
    _hists.th1_hists["h_prim_ke_pion_truth"] -> Fill ( utils::CalculateKE( rdr->primary_truth_Momentum.At(c_plane_), *rdr->primary_truth_Mass )*1.e3 );
    ...
    _hists.th1_hists["h_prim_pida_proton"] -> Fill( rdr->primaryPID_PIDA.At(c_plane_) );
    ...
    _hists.th1_hists["h_prim_pida_neutron"] -> Fill( rdr->primaryPID_PIDA.At(c_plane_) );
  }
    ...
    _hists.th1_hists["h_prim_pida_positron"] -> Fill( rdr->primaryPID_PIDA.At(c_plane_) );
  }
  else if ( *rdr->primary_truth_Pdg == utils::pdg::kPdgMuon ) {
    _hists.th1_hists["h_prim_pida_muon"] -> Fill( rdr->primaryPID_PIDA.At(c_plane_) );
...
void Pion_QE_MC_Selection::CalculateELoss( std::unique_ptr<Reader> & rdr, int daughter ) {
  ...
  double omega = rdr->primaryKineticEnergy.At(c_plane_) - rdr->daughterKineticEnergyP0.At(daughter);
